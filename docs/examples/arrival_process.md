...
Jumps have no Gaussian representation, so arrival processes need the particle filter.
This example simulates recurring importations into a seasonal SEIRS epidemic, then learns the unknown arrival rate online with the Liu-West filter, with the step-by-step loop written out.
...
```
(prior_mean = 0.008, final_posterior_mean = 0.0254, truth = 0.03)
```
...
fig
```
![](arrival_process-13.png)

The rate is identified only by the arrivals the data contain, and 120 days hold a handful.
The posterior moves up from the prior and its interval widens to take in the truth, but it stays wide; a longer series, or several locations sharing the rate, pins it down further.

The engine route, `build_inference(PF(n_particles = 4000), LiuWest(discount = 0.97), model; ...)`, runs the same loop from an [`EpiModel`](../api/configurableepi.md#ConfigurableEpi.EpiModel); a UKF or EnKF engine rejects a model with jump drivers.
